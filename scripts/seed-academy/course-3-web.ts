  ...
  modules: [
    // ─── Module 1 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "HTTP status 500 indicates:", options: ["Success", "A redirect", "A server-side error", "Not found"], correct: "A server-side error", explanation: "500 = server error; 404 = not found; 403 = forbidden." },
          { type: "MULTIPLE_CHOICE", question: "Returning 403 vs 404 for a resource can leak information because it reveals:", options: ["The server's IP", "Whether the resource exists", "The encryption key", "The user's password"], correct: "Whether the resource exists" },
          { type: "MULTIPLE_CHOICE", question: "The cookie flag that ensures a cookie is only sent over HTTPS is:", options: ["HttpOnly", "Secure", "Path", "Domain"], correct: "Secure", explanation: "HttpOnly blocks JS access; Secure restricts it to HTTPS." },
          { type: "MULTIPLE_CHOICE", question: "Which HTTP method is normally used to delete a resource?", options: ["GET", "POST", "DELETE", "HEAD"], correct: "DELETE" },
          { type: "TRUE_FALSE", question: "An attacker can forge any part of an HTTP request — path, headers, cookies, and body.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 2 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "Why are general-purpose hashes like SHA-256 a poor choice for passwords?", options: ["They are reversible", "They are far too fast, so attackers can try billions per second", "They can't be salted", "They aren't secure enough to compute"], correct: "They are far too fast, so attackers can try billions per second" },
          { type: "MULTIPLE_CHOICE", question: "The worst-case impact of storing passwords in plaintext is that a database leak:", options: ["Slows the site down", "Instantly exposes every account (and reused passwords elsewhere)", "Breaks HTTPS", "Deletes the sessions"], correct: "Instantly exposes every account (and reused passwords elsewhere)" },
          { type: "MULTIPLE_CHOICE", question: "A session token must be invalidated when the user does what?", options: ["Refreshes the page", "Logs out", "Scrolls down", "Changes their avatar"], correct: "Logs out" },
          { type: "MULTIPLE_CHOICE", question: "Even if a password leaks, which control most raises the bar for an attacker?", options: ["A prettier login page", "MFA", "Gzip compression", "A longer domain name"], correct: "MFA" },
          { type: "TRUE_FALSE", question: "A unique salt per password makes each stored hash different even for identical passwords.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 3 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "The root cause behind all injection bugs (SQLi included) is:", options: ["Weak passwords", "Data and code getting mixed together", "Missing HTTPS", "Slow servers"], correct: "Data and code getting mixed together" },
          { type: "MULTIPLE_CHOICE", question: "How does a parameterised query prevent SQLi?", options: ["It hides error messages", "It sends the query and the data separately so input is never treated as code", "It encrypts the database", "It rate-limits logins"], correct: "It sends the query and the data separately so input is never treated as code" },
          { type: "MULTIPLE_CHOICE", question: "Beyond parameterisation, a good defence-in-depth measure for SQLi is:", options: ["A least-privilege database account", "A brighter UI", "Disabling backups", "Longer cookies"], correct: "A least-privilege database account" },
          { type: "MULTIPLE_CHOICE", question: "With advanced payloads, SQL injection can allow an attacker to:", options: ["Only change fonts", "Extract entire databases or bypass logins", "Improve performance", "Encrypt traffic"], correct: "Extract entire databases or bypass logins" },
          { type: "TRUE_FALSE", question: "An ORM that parameterises queries by default helps prevent SQL injection.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 4 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "A crafted link that bounces a script off the server to run in the victim's browser is which type of XSS?", options: ["Stored XSS", "Reflected XSS", "DOM-based XSS", "SQL injection"], correct: "Reflected XSS" },
          { type: "MULTIPLE_CHOICE", question: "An XSS flaw that lives entirely in client-side JavaScript is called:", options: ["Stored XSS", "Reflected XSS", "DOM-based XSS", "CSRF"], correct: "DOM-based XSS" },
          { type: "MULTIPLE_CHOICE", question: "Why is XSS especially dangerous if the victim is an admin?", options: ["Admins have slower computers", "The script runs with the admin's privileges on the trusted site", "Admins can't use MFA", "It deletes the database"], correct: "The script runs with the admin's privileges on the trusted site" },
          { type: "MULTIPLE_CHOICE", question: "Output encoding stops XSS by ensuring user data is:", options: ["Deleted", "Rendered as inert text, not executable code", "Encrypted at rest", "Compressed"], correct: "Rendered as inert text, not executable code" },
          { type: "TRUE_FALSE", question: "React escapes rendered values by default, unless you use dangerouslySetInnerHTML.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 5 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "A feature that fetches a user-supplied URL ('import from URL') is a classic vector for:", options: ["SQL injection", "SSRF", "Password cracking", "Clickjacking"], correct: "SSRF" },
          { type: "MULTIPLE_CHOICE", question: "The 2019 Capital One breach involved SSRF against which target?", options: ["A public CDN", "The cloud instance metadata endpoint (169.254.169.254)", "A DNS root server", "The user's browser"], correct: "The cloud instance metadata endpoint (169.254.169.254)", explanation: "It exposed temporary credentials." },
          { type: "MULTIPLE_CHOICE", question: "Why are keyword deny-lists (e.g. blocking 'localhost') a weak SSRF defence?", options: ["They slow the server", "They're easily bypassed with redirects/DNS tricks/alternate encodings", "They block all traffic", "They require HTTPS"], correct: "They're easily bypassed with redirects/DNS tricks/alternate encodings" },
          { type: "TRUE_FALSE", question: "Network segmentation, so the app server can't reach sensitive internal systems, helps mitigate SSRF.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 6 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "The correct fix for IDOR is to:", options: ["Make IDs harder to guess", "Hide the button in the UI", "Authorise every request server-side against the specific object", "Use a longer session cookie"], correct: "Authorise every request server-side against the specific object", explanation: "Obscurity and UI hiding are not access control." },
          { type: "MULTIPLE_CHOICE", question: "Accessing another user's data at the same privilege level (e.g. via IDOR) is which kind of escalation?", options: ["Vertical", "Horizontal", "Diagonal", "None"], correct: "Horizontal" },
          { type: "MULTIPLE_CHOICE", question: "Which two controls best defend against CSRF?", options: ["Anti-CSRF tokens and the SameSite cookie attribute", "Gzip and caching", "Longer passwords and MFA", "Firewalls and backups"], correct: "Anti-CSRF tokens and the SameSite cookie attribute" },
          { type: "MULTIPLE_CHOICE", question: "A hidden form on a malicious page that changes your email on a site you're logged into is:", options: ["CSRF", "SSRF", "XSS", "SQLi"], correct: "CSRF" },
          { type: "TRUE_FALSE", question: "Broken Access Control is the current OWASP Top 10 #1 risk.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 7 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "Why is 'shift left' cheaper than fixing security issues later?", options: ["Developers work for free", "Fixing a flaw in design costs a fraction of fixing it in production", "It skips testing", "It removes the need for patching"], correct: "Fixing a flaw in design costs a fraction of fixing it in production" },
          { type: "MULTIPLE_CHOICE", question: "Which pair of practices prevents whole classes of injection and XSS bugs?", options: ["Validate input and encode output", "Compress and cache", "Log and reboot", "Rename and hide"], correct: "Validate input and encode output" },
          { type: "MULTIPLE_CHOICE", question: "A tool like `npm audit` or Dependabot supports which practice?", options: ["Software composition analysis", "Output encoding", "Rate limiting", "Session management"], correct: "Software composition analysis" },
          { type: "MULTIPLE_CHOICE", question: "Secrets such as API keys should be stored in:", options: ["Source code", "Environment variables or a secrets manager", "Public repos", "Code comments"], correct: "Environment variables or a secrets manager" },
          { type: "TRUE_FALSE", question: "'Don't roll your own crypto' means you should rely on vetted, well-reviewed libraries.", correct: "true" },
        ],
      },
    },
