  ...
  modules: [
    // ─── Module 1 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "The person who builds and tunes the detection rules a SOC relies on is a:", options: ["Detection Engineer", "Receptionist", "Tier 1 analyst", "Auditor"], correct: "Detection Engineer" },
          { type: "MULTIPLE_CHOICE", question: "An escalated alert needing deeper analysis and incident handling typically goes to:", options: ["Tier 1", "Tier 2", "The CEO", "Sales"], correct: "Tier 2", explanation: "Tier 1 triages and escalates; Tier 2 investigates in depth." },
          { type: "MULTIPLE_CHOICE", question: "SOCs run around the clock mainly because:", options: ["Staff prefer night shifts", "Attacks don't keep office hours", "Electricity is cheaper at night", "Logs only appear at night"], correct: "Attacks don't keep office hours" },
          { type: "TRUE_FALSE", question: "A strong Tier 1 analyst escalates real issues with clear notes rather than acting alone on complex incidents.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 2 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "Detection that matches a specific known-bad malware hash is which approach?", options: ["Signature-based", "Anomaly-based", "Baseline-based", "Random"], correct: "Signature-based", explanation: "Matching known-bad patterns is signature detection; deviations-from-normal is anomaly detection." },
          { type: "MULTIPLE_CHOICE", question: "A London-based finance user logging in at 3 a.m. from another country is flagged because it deviates from their:", options: ["Password", "Established baseline", "Firewall rule", "Job title"], correct: "Established baseline" },
          { type: "MULTIPLE_CHOICE", question: "Which telemetry source best answers 'who authenticated, from where, and when'?", options: ["Identity / auth events", "CPU temperature", "Disk SMART data", "Screen brightness"], correct: "Identity / auth events" },
          { type: "TRUE_FALSE", question: "Correlating more telemetry sources generally gives a clearer picture than any single source alone.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 3 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "In the entry 'Failed password for root from 203.0.113.45', what does 203.0.113.45 represent?", options: ["The target server", "The source IP the attempt came from", "The username", "The port"], correct: "The source IP the attempt came from" },
          { type: "MULTIPLE_CHOICE", question: "Windows Event ID 4688 records:", options: ["A failed logon", "A successful logon", "A new process being created", "A deleted file"], correct: "A new process being created", explanation: "4624 = success logon, 4625 = failed logon, 4688 = process creation." },
          { type: "MULTIPLE_CHOICE", question: "A typical log entry contains a timestamp, a source, an event type, and:", options: ["The details of the event", "An encryption key", "A firewall rule", "A backup copy"], correct: "The details of the event" },
          { type: "MULTIPLE_CHOICE", question: "In a brute-force log sequence, which line turns 'suspicious' into 'confirmed breach'?", options: ["The first failed password", "The 'Accepted password' success from the attacking IP", "A DNS lookup", "The hostname line"], correct: "The 'Accepted password' success from the attacking IP" },
          { type: "TRUE_FALSE", question: "DNS logs can help catch malware that 'calls home' to a command server.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 4 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "The single biggest value a SIEM adds over reading raw logs by hand is:", options: ["It deletes old logs", "It centralises and correlates logs and raises alerts", "It replaces analysts entirely", "It encrypts the network"], correct: "It centralises and correlates logs and raises alerts" },
          { type: "MULTIPLE_CHOICE", question: "Analysts start ignoring alerts because too many are false positives. This dangerous condition is called:", options: ["Alert fatigue", "Correlation", "Baselining", "Load balancing"], correct: "Alert fatigue" },
          { type: "MULTIPLE_CHOICE", question: "Which outcome is the most dangerous for a SOC?", options: ["False positive (benign alert)", "False negative (real attack, no alert)", "True positive (real alert)", "True negative (no alert, nothing wrong)"], correct: "False negative (real attack, no alert)", explanation: "A missed real attack goes completely unnoticed." },
          { type: "MULTIPLE_CHOICE", question: "Name a widely used SIEM platform.", options: ["Splunk", "Photoshop", "Excel", "nginx"], correct: "Splunk" },
          { type: "TRUE_FALSE", question: "A rule like 'if 20+ failed logins then a success from one IP, alert' is an example of correlation logic.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 5 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "Put these triage steps in order: gather context, understand the alert, decide, corroborate. Which comes SECOND?", options: ["Understand the alert", "Gather context", "Corroborate", "Decide"], correct: "Gather context", explanation: "Understand → gather context → corroborate → decide → document." },
          { type: "MULTIPLE_CHOICE", question: "You find one host with a known-bad IOC. The correct next move is to:", options: ["Assume it's the only one", "Sweep the environment for the same indicator elsewhere", "Delete the logs", "Ignore it"], correct: "Sweep the environment for the same indicator elsewhere", explanation: "One infected host is rarely the only one." },
          { type: "MULTIPLE_CHOICE", question: "A suspicious registry key set for persistence is which kind of indicator?", options: ["IOC (Indicator of Compromise)", "SLA", "Baseline", "Firewall rule"], correct: "IOC (Indicator of Compromise)" },
          { type: "MULTIPLE_CHOICE", question: "Why are TTP/behaviour-based indicators (IOAs) valuable alongside IOCs?", options: ["They are cheaper", "They can catch attacks in progress, before damage", "They never produce false positives", "They replace logging"], correct: "They can catch attacks in progress, before damage" },
          { type: "TRUE_FALSE", question: "An 'impossible travel' alert means one account logged in from two places too far apart to travel between in time.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 6 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "Removing the attacker's malware, accounts, and persistence — and closing the entry point — is which IR phase?", options: ["Containment", "Eradication", "Recovery", "Preparation"], correct: "Eradication" },
          { type: "MULTIPLE_CHOICE", question: "Restoring systems from known-clean backups and verifying they're clean is the ___ phase.", options: ["Identification", "Recovery", "Containment", "Preparation"], correct: "Recovery" },
          { type: "MULTIPLE_CHOICE", question: "Why must you preserve volatile memory before pulling the power?", options: ["It speeds up the reboot", "RAM holds evidence that vanishes on power-off", "It saves electricity", "It resets the malware"], correct: "RAM holds evidence that vanishes on power-off" },
          { type: "MULTIPLE_CHOICE", question: "Reimaging a compromised box immediately is risky because it:", options: ["Costs money", "Destroys evidence of how the attacker got in", "Improves security", "Contains the threat"], correct: "Destroys evidence of how the attacker got in", explanation: "Without knowing the entry point, the attacker may return the same way." },
          { type: "TRUE_FALSE", question: "Preparation — plans, tools, and training — happens before any incident occurs.", correct: "true" },
        ],
      },
    },
    ...
    // ─── Module 7 ───────────────────────────────────────────────
    {
      ...
      quiz: {
        ...
        questions: [
          { type: "MULTIPLE_CHOICE", question: "Why is detecting on TTPs more durable than detecting on simple IOCs?", options: ["TTPs are free", "Attackers can change IOCs easily but their techniques are harder to change", "IOCs never work", "TTPs are encrypted"], correct: "Attackers can change IOCs easily but their techniques are harder to change" },
          { type: "MULTIPLE_CHOICE", question: "'Strategic' threat intelligence is best described as:", options: ["Specific IP addresses to block now", "Big-picture insight like who is likely to target our industry", "A firewall rule", "A malware hash"], correct: "Big-picture insight like who is likely to target our industry", explanation: "Tactical intel is the specific IOCs; strategic intel is the big picture." },
          { type: "MULTIPLE_CHOICE", question: "A new report gives an IP tied to a ransomware crew. The best proactive use is to:", options: ["Ignore it unless breached", "Block it and retro-hunt logs for prior contact", "Email all staff", "Delete firewall rules"], correct: "Block it and retro-hunt logs for prior contact" },
          { type: "TRUE_FALSE", question: "A threat feed aimed at a totally different industry mostly just adds noise for your SOC.", correct: "true", explanation: "Intel must be relevant to be useful." },
        ],
      },
    },
