    ...
    <main className="min-h-screen bg-surface-0 flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Sign in to Sage Vault</h1>
        </div>

        {/* OAuth buttons */}
        <div className="space-y-3">
          <form action={async () => { "use server"; await signIn("google", { redirectTo: "/dashboard" }); }}>
            <button type="submit" className="w-full flex items-center justify-center gap-3 rounded-lg border border-edge bg-surface-2 px-4 py-3 text-sm font-medium text-white hover:bg-surface-2 transition-colors">
        ...
        <p className="text-center text-sm text-ink-3 mt-5">
          No account?{" "}
          <Link href="/sign-up" className="text-ok hover:text-ok transition-colors">
            Create one free
          </Link>
        </p>
      </div>
