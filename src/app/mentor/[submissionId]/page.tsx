      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <header className="mb-8">
          ...
          <div className="flex items-center gap-3 mt-3">
            <div className="h-7 w-7 rounded-full bg-ok-wash border border-ok-edge flex items-center justify-center text-xs font-bold text-ok shrink-0">
              {(submission.user.displayName ?? submission.user.email)[0].toUpperCase()}
            </div>
            <div>
              <p className="text-sm text-ink-2">{submission.user.displayName ?? submission.user.email}</p>
        ...
        <section className="mb-8">
          <h2 className="text-xs uppercase tracking-widest text-ink-3 mb-3">Assessment Instructions</h2>
        ...
        {/* Submission */}
        <section className="mb-8">
          <h2 className="text-xs uppercase tracking-widest text-ink-3 mb-3">Student Submission</h2>
          <div className="rounded-xl border border-edge p-5">
            {submission.type === "TEXT" ? (
              <p className="text-sm text-ink-2 leading-relaxed whitespace-pre-wrap">{submission.content}</p>
            ) : (
              <a
                href={submission.content}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm text-ok hover:underline break-all flex items-center gap-2"
              >
        ...
        {/* Review form */}
        <section>
          <h2 className="text-xs uppercase tracking-widest text-ink-3 mb-4">Your Review</h2>
          <ReviewForm
