  ...
  return (
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Compare the Requests" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">An order-import API accepts XML. Here's a normal request, followed by a malicious one:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-3">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{NORMAL_REQUEST}</pre>
        </div>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{MALICIOUS_REQUEST}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What is the &lt;!ENTITY xxe SYSTEM ...&gt; declaration?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — a DOCTYPE-declared external entity pointing at a file:// URI tells a vulnerable parser to read that file and substitute its contents wherever &xxe; is referenced. Flag: SAGE&#123;3xt3rn4l_3nt1ty_d3cl4r3d&#125;</p>
        )}
      </TaskShell>

      {/* Task 2 */}
      <TaskShell number={2} title="Confirm the Disclosure" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">The server's response to the malicious request:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SERVER_RESPONSE}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag which local file's contents were disclosed.</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — the "item" field now contains the contents of /etc/passwd instead of a product name. Flag: SAGE&#123;3tc_p4sswd_d1scl0s3d&#125;</p>
        )}
      </TaskShell>

      {/* Task 3 */}
      <TaskShell number={3} title="Fix the Parser" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">The dev team wants to keep accepting XML uploads, safely.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What is the correct fix?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;3xt3rn4l_3nt1ty_d3cl4r3d&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;3tc_p4sswd_d1scl0s3d&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;d1s4bl3_3xt3rn4l_3nt1ti3s&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
