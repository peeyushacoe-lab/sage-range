  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Find the Dangerous Permission" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">ACL excerpt on the domain object:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{ACL}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which permission, granted to a low-privilege account, enables DCSync?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — svc_reporting has "Replicating Directory Changes All" — the permission that enables DCSync. Flag: SAGE&#123;r3pl1c4t1ng_d1r3ct0ry_ch4ng3s_4ll&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Understand the Stealth" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">DCSync often goes undetected for far longer than a direct NTDS.dit theft.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why is DCSync so hard to detect compared to dumping the NTDS.dit file directly?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — it blends in as normal DC replication traffic rather than an obvious file-access event. Flag: SAGE&#123;l00ks_l1k3_l3g1t_r3pl1c4t10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Remediate the Finding" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">svc_reporting shouldn't have this permission at all.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the right remediation once this permission grant is found?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;r3pl1c4t1ng_d1r3ct0ry_ch4ng3s_4ll&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;l00ks_l1k3_l3g1t_r3pl1c4t10n&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;r3v0k3_r3pl_perm_4ud1t_0th3rs&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
