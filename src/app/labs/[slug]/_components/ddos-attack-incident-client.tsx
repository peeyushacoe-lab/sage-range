  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Confirm This Is a DDoS" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Traffic to the public login endpoint has spiked dramatically:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{TRAFFIC}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What confirms this is a DDoS rather than a legitimate traffic surge or internal failure?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — 50x normal baseline traffic from thousands of distinct IPs confirms a DDoS. Flag: SAGE&#123;50x_b4s3l1n3_thous4nds_0f_1ps&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Mitigate the Volume" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The service is going down under the load.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the fastest practical mitigation for a volumetric attack like this?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — a scrubbing service or CDN can absorb and filter the volume before your origin ever sees it. Flag: SAGE&#123;scrubb1ng_s3rv1c3_4bs0rbs_v0lum3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Watch for What's Hidden" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">Mitigation is in place, but the login endpoint's own rate limiting is still active too.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why is it important to keep the login endpoint's own rate limiting in place even after DDoS mitigation kicks in?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;50x_b4s3l1n3_thous4nds_0f_1ps&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;scrubb1ng_s3rv1c3_4bs0rbs_v0lum3&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;d0s_c4n_h1d3_cr3d_stuff1ng&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
