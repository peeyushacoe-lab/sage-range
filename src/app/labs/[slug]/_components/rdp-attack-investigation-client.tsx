  ...
  return (
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Detect the Brute Force" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — 1,204 failures across multiple accounts from one IP, ending in success, is textbook RDP brute force. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Identify the Attacker's Action" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — `net user backup_svc ... /add` creates a new local account for later use. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Confirm the Persistence" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        {done("task_3") && (
          <p className="text-sm font-mono text-sage-400">Correct — backup_svc was added to the local Administrators group, giving the attacker durable admin-level persistence. Flag: {revealed.task_3 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
