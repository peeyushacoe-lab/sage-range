  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Resolve Conflicting Entries" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Two threat intel feeds return conflicting data on the same IP:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{FEEDS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which value should you trust for 45.33.10.12, and why?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — the more recent, higher-confidence report should win over an older, lower-confidence one. Flag: SAGE&#123;h1gh_c0nf1d3nc3_r3c3nt_4g3_w1ns&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Decide the Domain's Treatment" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The domain cdn-edge-01[.]net has only medium confidence from a single feed, seen 6 hours ago.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the right integration action for this indicator?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — monitor it first; a single medium-confidence source isn't a strong enough bar for automatic enforcement. Flag: SAGE&#123;m0n1t0r_f1rst_n0t_4ut0_bl0ck&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Set the Auto-Block Bar" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You need a written policy for when an indicator can skip human review and go straight to auto-block.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the right criteria for auto-blocking an IOC with no human review?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;h1gh_c0nf1d3nc3_r3c3nt_4g3_w1ns&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;m0n1t0r_f1rst_n0t_4ut0_bl0ck&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;h1gh_c0nf_multi_f33d_4ut0bl0ck&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
