  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Spot the Pattern" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Login activity from two different windows on the same environment:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{LOGIN_DATA}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What two conditions, combined, distinguish credential stuffing from normal noise here?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — high failed-login volume combined with a large number of distinct usernames from one source. Flag: SAGE&#123;h1gh_v0lum3_4nd_un1qu3_us3rs&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Choose the Logic Operator" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">You're writing the actual detection rule using these two conditions as inputs.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Should the rule use AND or OR logic between the two conditions?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — requiring both conditions together (AND) keeps normal noise from tripping the rule on its own. Flag: SAGE&#123;4nd_l0g1c_n0t_0r&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Set the Thresholds" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">A fixed number like "500 failed logins" would work today, but every environment and time of day differs.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What threshold-tuning approach avoids both false positives and missed detections?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;h1gh_v0lum3_4nd_un1qu3_us3rs&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;4nd_l0g1c_n0t_0r&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;b4s3l1n3_dr1v3n_thr3sh0ld&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
