    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Email Header Analysis" unlocked completed={done("task_1")}>
        ...
        <div className="rounded-lg border border-edge overflow-hidden mb-4">
          <div className="px-3 py-2 bg-surface-1 border-b border-edge-subtle text-xs text-ink-3 font-mono">
          ...
          <div className="divide-y divide-edge-subtle">
            {EMAIL_HEADER_FIELDS.map((h) => {
              ...
              return (
                <div key={h.field}>
                  <button
                    className="w-full text-left px-3 py-2.5 font-mono text-xs hover:bg-surface-2 transition-colors flex items-start gap-3 group"
                    onClick={() => examineHeader(h.field)}
                  >
                    <span className="text-ink-3 shrink-0 w-36 pt-px">{h.field}:</span>
                    <span className={`flex-1 break-all ${
                      isExamined && h.suspicious ? "text-danger" : "text-warn"
                    }`}>{h.value}</span>
                    {isExamined ? (
                      ...
                      <span className="shrink-0 text-[9px] text-ink-3 group-hover:text-ink-2">click to analyze</span>
                    )}
                  </button>
                  {isExamined && (
                    <div className="px-3 py-2.5 bg-surface-0 border-t border-edge-subtle text-xs text-ink-2 leading-relaxed">
                      {h.analysis}
                    </div>
                  )}
                </div>
        ...
        {allSuspiciousExamined && !done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What is the originating IP of this phishing email?</p>
      ...
      <TaskShell number={2} title="Domain Registration Lookup" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        <div className="rounded-lg border border-edge overflow-hidden mb-4">
          <div className="px-3 py-2 bg-surface-1 border-b border-edge-subtle text-xs font-mono grid grid-cols-4 gap-2 text-ink-3">
          ...
          <div className="divide-y divide-edge-subtle">
            {PIVOT_DOMAINS.map((d) => (
              <div key={d.domain}>
                <button
                  className="w-full text-left px-3 py-2.5 font-mono text-xs hover:bg-surface-2 transition-colors grid grid-cols-4 gap-2"
                  onClick={() => setExpandedDomain(expandedDomain === d.domain ? null : d.domain)}
                >
                  <span className="text-warn truncate">{d.domain}</span>
                  <span className="text-danger truncate">{d.ns.split(".")[0]}...</span>
                  <span className="text-ink-2">{d.country}</span>
                  <span className="text-ink-2 flex items-center gap-2">
                    {d.created}
                    <span className="text-ink-3">{expandedDomain === d.domain ? "▲" : "▼"}</span>
                  </span>
                </button>
                {expandedDomain === d.domain && (
                  <div className="px-4 py-3 bg-surface-0 border-t border-edge-subtle">
                    <div className="grid grid-cols-2 gap-4 text-xs mb-3">
                      <div className="space-y-1.5">
                        <p><span className="text-ink-3">Domain: </span><span className="text-warn font-mono">{d.domain}</span></p>
                        <p><span className="text-ink-3">Registrar: </span><span className="text-ink-2">{d.registrar}</span></p>
                        <p><span className="text-ink-3">Registrant Country: </span><span className="text-ink-2">{d.country}</span></p>
                        <p><span className="text-ink-3">Created: </span><span className="text-ink-2">{d.created}</span></p>
                      </div>
                      <div className="space-y-1.5">
                        <p><span className="text-ink-3">NS1: </span><span className="text-danger font-mono">{d.ns}</span></p>
                        <p><span className="text-ink-3">NS2: </span><span className="text-danger font-mono">{d.ns.replace("ns1", "ns2")}</span></p>
                        <p><span className="text-ink-3">DNSSEC: </span><span className="text-ink-2">unsigned</span></p>
                      </div>
                    </div>
                    <div className="rounded border border-warn-edge bg-warn-wash px-3 py-2 text-xs text-warn">
                      {d.detail}
                    </div>
                  </div>
                )}
        ...
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What nameserver is shared by all these malicious domains?</p>
      ...
      <TaskShell number={3} title="Infrastructure Pivot" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <p className="text-xs text-ink-3 mb-3">Infrastructure cluster — ns1.bulletproof-hosting.biz</p>
          <div className="space-y-1.5">
            {PIVOT_DOMAINS.map((d, i) => (
              <div key={d.domain} className="flex items-center gap-3 text-xs font-mono">
                <span className="text-ink-3 w-4 text-right shrink-0">{i + 1}.</span>
                <span className="text-warn">{d.domain}</span>
                <span className="text-ink-3">→</span>
                <span className="text-danger truncate">{d.ns}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-ink-3 mt-3 pt-3 border-t border-edge-subtle">
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;91.108.4.33&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;ns1.bulletproof-hosting.biz&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4_d0m41ns_s4me_1nfr4&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
