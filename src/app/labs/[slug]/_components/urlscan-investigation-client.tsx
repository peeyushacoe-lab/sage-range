  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Read the Scan Report" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A reported link comes back with this urlscan.io report:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SCAN_REPORT}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the final landing domain and the brand it impersonates.</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — secure-0ffice365-login.com impersonates the Office 365 sign-in page. Flag: SAGE&#123;secur30ff1c3365l0g1n_0ff1c3365_1mp3rs0n4t10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Follow the Redirect Chain" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The page has a valid HTTPS certificate and a normal-looking login form.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What single detail about this page is the strongest phishing indicator?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — layered redirects through a shortener and relay, landing on a two-day-old lookalike domain, is far more telling than the padlock. Flag: SAGE&#123;mult1_h0p_r3d1r3ct_ch41n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Take Action" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">The page has a 92/100 malicious verdict and is live right now.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What should the SOC do right now?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;secur30ff1c3365l0g1n_0ff1c3365_1mp3rs0n4t10n&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;mult1_h0p_r3d1r3ct_ch41n&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;t4k3d0wn_4nd_pr0xy_bl0ck&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
