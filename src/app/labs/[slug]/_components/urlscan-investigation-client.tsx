  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Read the Scan Report" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — secure-0ffice365-login.com impersonates the Office 365 sign-in page. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Follow the Redirect Chain" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — layered redirects through a shortener and relay, landing on a two-day-old lookalike domain, is far more telling than the padlock. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
