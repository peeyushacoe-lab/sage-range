    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Spot the Obfuscation Flag" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">
          EDR flagged this PowerShell command line spawned from <code className="text-warn">cmd.exe</code> on
          <code className="text-warn"> DESKTOP-FIN02</code>.
        </p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4 overflow-x-auto">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap break-all">{ENCODED_CMD}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which flag hides the true command from casual log review?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — -Enc takes a Base64-encoded UTF-16LE string, hiding the real command from plain-text log scanning. Flag: SAGE&#123;3nc0d3d_p0w3rsh3ll_fl4g&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Decode the Payload" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{AMSI_LOG}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Decode the command. What URL was it about to download from?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — the command was a download-cradle fetching a second-stage script from 198.51.100.42/p.ps1. Flag: SAGE&#123;d0wnl04d_cr4dl3_f0und&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Find the Persistence" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{PROFILE_LOG}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What persistence mechanism is this?</p>
            <div className="flex flex-wrap gap-3">
        ...
        {done("task_3") && (
          <p className="text-sm font-mono text-ok">Correct — profile.ps1 executes automatically on every new PowerShell session, re-fetching the payload each time. Flag: SAGE&#123;pr0fil3_p3rsist3nc3&#125;</p>
        )}
      </TaskShell>
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;3nc0d3d_p0w3rsh3ll_fl4g&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;d0wnl04d_cr4dl3_f0und&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;pr0fil3_p3rsist3nc3&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
