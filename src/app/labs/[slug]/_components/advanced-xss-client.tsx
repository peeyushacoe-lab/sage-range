  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Bypass the Filter" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A comment field's server-side sanitization, and a payload that got through:</p>
        ...
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What payload bypasses this naive &lt;script&gt; tag filter?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — &lt;img src=x onerror=...&gt; never contains "script" and still executes JS. Flag: SAGE&#123;1mg_0n3rr0r_byp4ss3s_f1lt3r&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Understand the Filter Gap" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The filter blocks only the literal string &lt;script&gt;.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why does a filter that only blocks the literal &lt;script&gt; string fail here?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — any element with an inline event handler attribute can execute script, no &lt;script&gt; tag needed. Flag: SAGE&#123;3v3nt_h4ndl3rs_n0_scr1pt_n33d3d&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Spot the DOM-Based Sink" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">A separate part of the app has this client-side code:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{DOM_SINK}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why can server-side input sanitization completely miss this vulnerability?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;1mg_0n3rr0r_byp4ss3s_f1lt3r&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;3v3nt_h4ndl3rs_n0_scr1pt_n33d3d&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;cl13nt_s1d3_0nly_n3v3r_s3rv3r&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
