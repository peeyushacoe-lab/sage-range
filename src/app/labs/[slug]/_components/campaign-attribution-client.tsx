  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Weigh the Evidence" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Evidence gathered across a multi-stage campaign investigation:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{EVIDENCE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Using an admiralty-style confidence approach, what's the overall attribution confidence level here — low, moderate, or high?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — combined, this mix of medium and high confidence indicators supports a moderate overall confidence level. Flag: SAGE&#123;m0d3r4t3_c0nf1d3nc3_4ttr1but10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Question the Strongest Indicator" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">A manager wants to attribute this immediately based purely on the custom malware match.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why shouldn't a single high-confidence indicator (like the custom malware) alone justify full attribution?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — custom malware can be shared, leaked, or false-flagged, so it needs corroboration from other evidence. Flag: SAGE&#123;c0rr0b0r4t3_4cr0ss_3v1d3nc3_typ3s&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Write the Report Responsibly" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">Time to communicate this attribution to stakeholders.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the responsible way to communicate this attribution in a report to stakeholders?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;m0d3r4t3_c0nf1d3nc3_4ttr1but10n&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;c0rr0b0r4t3_4cr0ss_3v1d3nc3_typ3s&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;st4t3_c0nf1d3nc3_3xpl1c1tly&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
