  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Weigh the Evidence" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — combined, this mix of medium and high confidence indicators supports a moderate overall confidence level. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Question the Strongest Indicator" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — custom malware can be shared, leaked, or false-flagged, so it needs corroboration from other evidence. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
