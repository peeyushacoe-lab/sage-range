    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Identify the Jailbreak Technique" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">
          <code className="text-warn">ContentModAI</code> is supposed to refuse requests for harmful instructions.
        </p>
        ...
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What technique got the model to comply?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — wrapping the request in a fictional persona ("LockGPT" in a novel scene) is a classic roleplay jailbreak that talks the model out of its refusal behavior. Flag: SAGE&#123;r0l3pl4y_j41lbr34k&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Assess the Real Harm" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why is that reasoning flawed?</p>
            <div className="flex flex-col gap-2">
              {[
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — the "character" label is just packaging; the literal, usable, technically-accurate instructions still get delivered to the user regardless of the story wrapped around them. Flag: SAGE&#123;f1ct10n_1s_p4ck4g1ng_0nly&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Pick the Right Defense" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the defense that inspects actual output content, rather than trusting the request's framing.</p>
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;r0l3pl4y_j41lbr34k&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;f1ct10n_1s_p4ck4g1ng_0nly&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;0utput_f1lt3r1ng_n0t_p3rs0n4_trust&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
