  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Read the Pulse" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — 27 indicators covering the QuietPanda backdoor family. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Judge Credibility" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — verification, subscriber corroboration, and references are what make a pulse actionable, not its TLP marking or indicator count alone. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Hunt Your Own Logs" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        {done("task_3") && (
          <p className="text-sm font-mono text-sage-400">Correct — WKSTN-FIN-03 contacted 91.223.10.15, one of the pulse's flagged IPs. Flag: {revealed.task_3 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
