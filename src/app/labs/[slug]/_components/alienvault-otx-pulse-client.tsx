  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Read the Pulse" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A colleague shares an OTX pulse that just showed up in your feed:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{PULSE_A}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the total indicator count and the malware family this pulse covers.</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — 27 indicators covering the QuietPanda backdoor family. Flag: SAGE&#123;27_1nd1c4t0rs_qu13tp4nd4&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Judge Credibility" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">A second pulse on the same topic surfaces. Compare the two before deciding what to act on:</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          <div className="rounded-lg bg-surface-0 border border-edge p-4">
            <p className="text-xs text-ink-3 mb-2">Pulse A</p>
            <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{PULSE_A}</pre>
          </div>
          <div className="rounded-lg bg-surface-0 border border-edge p-4">
            <p className="text-xs text-ink-3 mb-2">Pulse B</p>
            <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{PULSE_B}</pre>
          </div>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which pulse should you prioritize acting on first, and why?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — verification, subscriber corroboration, and references are what make a pulse actionable, not its TLP marking or indicator count alone. Flag: SAGE&#123;v3r1f13d_h1gh_c0nf1d3nc3_puls3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Hunt Your Own Logs" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">You pull a subset of Pulse A's indicators and sweep your firewall logs against them:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-3">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{IOC_LIST}</pre>
        </div>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-ink-2 whitespace-pre-wrap overflow-x-auto">{FIREWALL_LOG}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which host contacted a pulse indicator, and which one?</p>
            <div className="flex gap-2 max-w-md">
        ...
        {done("task_3") && (
          <p className="text-sm font-mono text-ok">Correct — WKSTN-FIN-03 contacted 91.223.10.15, one of the pulse's flagged IPs. Flag: SAGE&#123;wkstn_f1n_03_m4tch3d_pulse_10c&#125;</p>
        )}
      </TaskShell>
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;27_1nd1c4t0rs_qu13tp4nd4&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;v3r1f13d_h1gh_c0nf1d3nc3_puls3&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;wkstn_f1n_03_m4tch3d_pulse_10c&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
