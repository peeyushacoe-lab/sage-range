  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Measure the Rule" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A week-long validation run for a new detection rule:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{VALIDATION_RESULTS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What is this rule's false-positive rate?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — 334 of 340 alerts were false positives, roughly 98%. Flag: SAGE&#123;98_p3rc3nt_f4ls3_p0s1t1v3_r4t3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Decide on Blocking Mode" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">Your team is considering switching this rule from alert-only to auto-response (blocking) mode.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Is this rule ready to run in blocking/auto-response mode?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — at 98% false positives, blocking mode would break legitimate admin work far more often than it stops real attacks. Flag: SAGE&#123;n0t_r34dy_f0r_4ut0_bl0ck&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Tune Before Reconsidering" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You want to fix the rule instead of abandoning it entirely.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the right next step before reconsidering blocking mode?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;98_p3rc3nt_f4ls3_p0s1t1v3_r4t3&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;n0t_r34dy_f0r_4ut0_bl0ck&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;3xclud3_kn0wn_g00d_r3b4s3l1n3&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
