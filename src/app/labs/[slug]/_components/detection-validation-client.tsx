  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Measure the Rule" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — 334 of 340 alerts were false positives, roughly 98%. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Decide on Blocking Mode" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — at 98% false positives, blocking mode would break legitimate admin work far more often than it stops real attacks. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
