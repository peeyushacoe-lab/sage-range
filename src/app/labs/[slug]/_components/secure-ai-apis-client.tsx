  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Find the Key Leak" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A code review turns up this in the production JS bundle:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{BUNDLE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Where should the API key live instead?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — the key must live server-side only, never in code shipped to the browser. Flag: SAGE&#123;4p1_k3y_s3rv3r_s1d3_0nly&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Stop the Cost DoS" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">A billing alert fires shortly after the fix:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{COST_SCENARIO}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What control addresses this cost-based denial of service risk?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — per-user rate limiting and quotas cap what any single account can spend. Flag: SAGE&#123;r4t3_l1m1t_pr3v3nts_c0st_d0s&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Round Out the Hardening" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">The key is secured and rate limiting is in place.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Besides rate limiting, what else should validate every incoming request to the AI API?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;4p1_k3y_s3rv3r_s1d3_0nly&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;r4t3_l1m1t_pr3v3nts_c0st_d0s&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;v4l1d4t3_1nput_f1lt3r_0utput&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
