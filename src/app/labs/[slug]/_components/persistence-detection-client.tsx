  ...
  return (
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Audit Scheduled Tasks" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Three scheduled tasks exist on a workstation. One doesn&apos;t point where it claims to.</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SCHEDULED_TASKS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which task is suspicious?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — a real OneDrive updater task would point into the OneDrive install directory, not C:\Windows\Temp\svc.exe. Flag: SAGE&#123;m4sq3r4d1ng_sch3dul3d_t4sk&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Audit the Run Key" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{REGISTRY_RUN}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which Run key entry is suspicious?</p>
            <div className="flex flex-col gap-2">
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — svchost32.exe in Roaming AppData with a generic "WinUpdate32" name is a classic persistence disguise. Flag: SAGE&#123;run_k3y_p3rsist3nc3&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Confirm the Service Persistence" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{SERVICE_LOG}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the binary this rogue service points to, and what it's masquerading as (a Windows Update tool).</p>
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;m4sq3r4d1ng_sch3dul3d_t4sk&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;run_k3y_p3rsist3nc3&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;wupdmgr_svc_masquerade&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
