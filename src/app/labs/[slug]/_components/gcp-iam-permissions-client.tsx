  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Audit the IAM Bindings" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">You export the IAM policy for a production GCP project:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{IAM_BINDINGS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which binding grants access to anyone with any Google account, not just your org?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — allAuthenticatedUsers means any Google account anywhere on the internet, not just your organization. Flag: SAGE&#123;4ll4uth3nt1c4t3dus3rs_v13w3r_3xp0sur3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Check the CI Service Account" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The CI pipeline's job is only to deploy to Cloud Run and push images to Artifact Registry.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What is roles/editor's actual scope compared to what CI deploy needs?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — Editor grants broad write access across almost every service in the project, far more than a deploy pipeline needs. Flag: SAGE&#123;3d1t0r_t00_br04d_f0r_c1_d3pl0y&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Fix the Service Account" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You need to tighten the CI service account without breaking its deploy pipeline.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the correct fix for the CI service account?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;4ll4uth3nt1c4t3dus3rs_v13w3r_3xp0sur3&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;3d1t0r_t00_br04d_f0r_c1_d3pl0y&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;cust0m_r0l3_c1_sc0p3d&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
