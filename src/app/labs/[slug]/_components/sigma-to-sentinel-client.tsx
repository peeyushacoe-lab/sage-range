  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Map the Data Source" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">The Sigma rule you need to port to Sentinel:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SIGMA_RULE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which Sentinel/Log Analytics table does a Sigma process_creation rule map to?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — process_creation events land in DeviceProcessEvents. Flag: SAGE&#123;d3v1c3pr0c3ss3v3nts_t4bl3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Review a Rough Translation" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">A teammate's first-pass KQL translation:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{KQL_DRAFT}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's wrong with this direct translation, compared to the original Sigma rule?</p>
            <div className="flex flex-col gap-2">
              {[
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — the Sigma rule required the javascript: argument too; dropping that condition makes the KQL version far too broad. Flag: SAGE&#123;t00_br0ad_kql_transl4t10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Fix the Query" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You need to bring the KQL version back in line with the original Sigma rule's intent.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What should the fixed KQL add to reduce false positives while still catching the technique?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;d3v1c3pr0c3ss3v3nts_t4bl3&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;t00_br0ad_kql_transl4t10n&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4rgum3nt_sc0p3d_d3t3ct10n&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
