  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Trace the Initial Access" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">3:14 AM — an EDR alert fires on a finance workstation:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{PHISHING_ALERT}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What was the initial access vector and delivery mechanism?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — a macro-enabled spreadsheet, opened from a spoofed helpdesk email, spawned encoded PowerShell the moment it was opened. Flag: SAGE&#123;macr0_3n4bl3d_x1sm_ph1sh1ng&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Follow Lateral Movement" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">Minutes later, activity spreads to the primary file server:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{LATERAL_MOVEMENT}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">How did the attacker reach the file server from a single compromised workstation?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — a cached domain admin token on the compromised workstation let the attacker connect straight to FS01's admin$ share, no exploit needed. Flag: SAGE&#123;c4ch3d_d0m41n_4dm1n_r3us3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Contain and Recover" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">4,812 files are now encrypted, and this note has been dropped across the shared drive:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap">{RANSOM_NOTE}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What is the immediate, correct containment and recovery step?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          <h3 className="font-bold text-ok text-base">Incident Closed</h3>
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;macr0_3n4bl3d_x1sm_ph1sh1ng&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;c4ch3d_d0m41n_4dm1n_r3us3&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;1s0l4t3_r3st0r3_d0nt_p4y_f1rst&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
