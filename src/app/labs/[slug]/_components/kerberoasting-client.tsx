  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Identify the Weak Ticket" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">SPN-registered accounts and their ticket encryption types:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SPN_LIST}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which account's ticket is crackable offline due to weak encryption, and what type?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — svc_sql's ticket uses weak RC4 encryption, crackable offline. Flag: SAGE&#123;svc_sql_rc4_crack4bl3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Explain the Target Choice" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">Kerberoasting attackers overwhelmingly target service accounts over regular users.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why are service accounts especially attractive Kerberoasting targets?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — stale, rarely-rotated passwords make service accounts prime Kerberoasting targets. Flag: SAGE&#123;st4l3_s3rv1c3_4cc0unt_pw&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Recommend the Fix" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">Time to write up the mitigation.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the most effective mitigation against Kerberoasting?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;svc_sql_rc4_crack4bl3&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;st4l3_s3rv1c3_4cc0unt_pw&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4es_0nly_l0ng_r4nd0m_pw&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
