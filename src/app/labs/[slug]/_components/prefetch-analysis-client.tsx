  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Spot the Outlier" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Prefetch entries pulled from a suspected compromised workstation:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{PREFETCH_TABLE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which executable's run count makes it stand out as suspicious?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — update_helper.exe has only ever run once, unlike the everyday tools around it. Flag: SAGE&#123;upd4t3_h3lp3r_run_c0unt_1&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Check the Execution Path" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">You pull the referenced path from that prefetch entry:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{PATH_DETAIL}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What about this path is itself suspicious?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — a world-writable directory like C:\Users\Public is a classic drop location for malware, not a real install path. Flag: SAGE&#123;public_f0ld3r_3x3cut10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Correlate the Timeline" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">A phishing alert from the same host, around the same time:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{PHISHING_ALERT}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What does the matching timestamp between the alert and the prefetch entry prove?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;upd4t3_h3lp3r_run_c0unt_1&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;public_f0ld3r_3x3cut10n&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;pr3f3tch_c0nf1rms_3ntry_p01nt&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
