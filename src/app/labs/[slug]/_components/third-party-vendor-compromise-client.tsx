  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Assess the Exposure" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — you must assume the vendor's credentials into your environment are compromised too. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Act on Your Side Now" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — revoke or disable the vendor's access now rather than waiting on their own investigation timeline. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
