  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Spot the Data Risk" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">The architecture of an internally-deployed LLM app:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{ARCHITECTURE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What data risk exists purely from the choice of training data here?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — customer PII from the tickets can end up memorized inside the fine-tuned model. Flag: SAGE&#123;p11_m3m0r1z3d_1n_f1n3tun3d_m0d3l&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Prioritize the Finding" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The assessment turns up several findings — this one needs to be triaged first.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which assessment finding is most urgent to fix first?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — any employee querying the model could trigger it to leak another customer's memorized PII. Flag: SAGE&#123;m0d3l_c4n_l34k_tr41n1ng_p11&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Recommend a Mitigation" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">Retraining the model on scrubbed data will take months.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's a mitigation for training-data memorization leakage besides retraining?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;p11_m3m0r1z3d_1n_f1n3tun3d_m0d3l&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;m0d3l_c4n_l34k_tr41n1ng_p11&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;0utput_dlp_f1lt3r1ng&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
