  ...
  return (
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Map the Opening Move" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — a spearphishing attachment is how the attacker first got in (TA0001). Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Identify the Credential Access Technique" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — dumping credentials from LSASS memory is T1003.001, under the Credential Access tactic (TA0006). Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Map the Persistence Mechanism" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        {done("task_3") && (
          <p className="text-sm font-mono text-sage-400">Correct — a scheduled task set to run at logon is T1053 (Scheduled Task/Job), under Persistence (TA0003). Flag: {revealed.task_3 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      {allDone && (
        <div className="rounded-lg border border-sage-500/40 bg-sage-500/5 p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-zinc-500">Initial Access —</span> <span className="text-sage-400">{revealed.task_1 ?? "SAGE{…}"}</span></li>
            <li><span className="text-zinc-500">Credential Access —</span> <span className="text-sage-400">{revealed.task_2 ?? "SAGE{…}"}</span></li>
            <li><span className="text-zinc-500">Persistence —</span> <span className="text-sage-400">{revealed.task_3 ?? "SAGE{…}"}</span></li>
          </ul>
        </div>
      )}
    </div>
