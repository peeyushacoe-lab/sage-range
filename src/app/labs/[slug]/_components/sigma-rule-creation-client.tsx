    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Choose the Log Source" unlocked completed={done("task_1")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SAMPLE_LOGS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which Sigma logsource category applies here?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — Event ID 4688 is a new-process event, mapping to the process_creation logsource. Flag: SAGE&#123;pr0c3ss_cr34t10n_l0gs0urc3&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Find the Distinguishing Field" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{SIGMA_SKELETON}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What substring should fill in the CommandLine|contains field?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — payload.dll,Entry is the distinguishing substring; the legitimate rundll32 calls reference known system DLLs like shell32.dll. Flag: SAGE&#123;payload_dll_entry&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="Critique the Rule" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">
          A teammate suggests simplifying the rule to just <code className="text-warn">Image|endswith: &apos;\\rundll32.exe&apos;</code> with
          no CommandLine condition at all, to &quot;catch everything.&quot;
        </p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What&apos;s wrong with that simplification?</p>
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;pr0c3ss_cr34t10n_l0gs0urc3&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;payload_dll_entry&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;t00_br0ad_f4ls3_p0s1t1v3s&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
