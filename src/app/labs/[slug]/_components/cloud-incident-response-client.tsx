  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Stop the Bleeding" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">An active exfiltration alert fires right now:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{LIVE_ALERT}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What's the very first containment action that stops the bleeding without destroying evidence?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — revoking the compromised credentials immediately stops the exfiltration in progress. Flag: SAGE&#123;r3v0k3_cr3d3nt14ls_1mm3d14t3ly&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Resist the Urge to Tear Down" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">A teammate suggests immediately deleting the compromised bucket and terminating any related instances.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why not just delete/terminate the compromised resources right away?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — tearing down resources destroys the volatile evidence needed to scope the incident and find root cause. Flag: SAGE&#123;pr3s3rv3_3v1d3nc3_b3f0r3_t34rd0wn&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Determine the True Scope" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">Credentials are revoked and evidence is preserved.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">After containment, what determines the true scope of the breach in a cloud environment?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;r3v0k3_cr3d3nt14ls_1mm3d14t3ly&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;pr3s3rv3_3v1d3nc3_b3f0r3_t34rd0wn&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4ud1t_4ll_4ct10ns_n0t_just_0n3_s3rv1c3&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
