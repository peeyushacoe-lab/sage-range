  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Map the Tactic" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — scheduled tasks that survive reboot fall under the Persistence tactic. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Spot the Coverage Gap" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — zero coverage for a tactic that clearly occurred is a real detection blind spot needing new rules. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
