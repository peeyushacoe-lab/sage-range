  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Build a Smarter Wordlist" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — a targeted wordlist matching the known pattern avoids tripping the lockout threshold. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Read the Result" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — jsmith / Summer2026! is the valid credential pair. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
