  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Build a Smarter Wordlist" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Engagement details and password policy intel:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{POLICY}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Given the lockout policy and the disclosed password pattern, what's the smarter approach vs a generic wordlist?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — a targeted wordlist matching the known pattern avoids tripping the lockout threshold. Flag: SAGE&#123;t4rg3t3d_w0rdl1st_4v01ds_l0ck0ut&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Read the Result" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">Hydra output after running the targeted wordlist:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-ok whitespace-pre-wrap overflow-x-auto">{HYDRA_OUTPUT}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What username/password combination was found valid?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — jsmith / Summer2026! is the valid credential pair. Flag: SAGE&#123;jsm1th_summ3r2026_val1d&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Know When to Stop" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You now have one working credential.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What should you NOT do next?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;t4rg3t3d_w0rdl1st_4v01ds_l0ck0ut&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;jsm1th_summ3r2026_val1d&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;st0p_spr4y1ng_4ft3r_h1t&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
