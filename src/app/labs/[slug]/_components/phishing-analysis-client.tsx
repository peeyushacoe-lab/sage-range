    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Header Forgery" unlocked completed={done("task_1")}>
        ...
        <div className="rounded-lg border border-edge overflow-hidden mb-4">
          <p className="px-3 py-1.5 text-[10px] font-mono text-ink-3 bg-surface-1 border-b border-edge-subtle uppercase tracking-widest">
          ...
          {EMAIL_HEADERS.map((h) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-2 border-b border-edge-subtle last:border-0 transition-all ${
                  examined
                    ? h.suspicious
                      ? "bg-danger-wash"
                      : "bg-surface-1"
              ...
              >
                <div className="flex items-start gap-3">
                  <span className="font-mono text-xs text-ink-3 shrink-0 w-36 pt-px">{h.field}:</span>
                  <span className={`font-mono text-xs break-all ${examined && h.suspicious ? "text-danger" : "text-ink-2"}`}>
                    {h.value}
                  </span>
                  {examined && h.suspicious && (
                    <span className="shrink-0 text-[9px] font-bold text-danger border border-danger-edge rounded px-1 py-px ml-auto">SUSPICIOUS</span>
                  )}
                  {examined && !h.suspicious && (
                    <span className="shrink-0 text-[9px] font-bold text-ink-3 border border-edge-strong rounded px-1 py-px ml-auto">OK</span>
                  )}
                </div>
                {examined && (
                  <p className={`text-[10px] mt-1.5 leading-snug pl-[156px] ${h.suspicious ? "text-danger" : "text-ink-3"}`}>
                    {h.analysis}
                  </p>
                )}
              </button>
            ...
            <>
              ...
              {allSuspiciousExamined && !done("task_1") && (
                <form onSubmit={submitT1} className="space-y-3 mt-2">
                  <p className="text-sm text-ink-2 font-medium">All suspicious headers identified. How many distinct indicators confirm this is spoofed?</p>
      ...
      <TaskShell number={2} title="URL Deobfuscation" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">
          ...
          (replacing <code className="font-mono text-warn">https</code> with <code className="font-mono text-warn">hxxps</code>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <p className="font-mono text-xs text-ink-3 mb-2">Obfuscated URL from email body:</p>
          <code className="font-mono text-sm text-warn break-all">
            hxxps://g00gle[.]c0m.malicious-domain[.]ru/update
          </code>
          <p className="text-xs text-ink-3 mt-3">
            Defanging notation: <code className="font-mono text-warn">hxxps</code> = https,
            <code className="font-mono text-warn"> [.]</code> = literal dot.
            The fake Google prefix is designed to deceive at a glance.
          </p>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What is the actual malicious domain?</p>
      ...
      <TaskShell number={3} title="Macro Behavior" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <p className="font-mono text-xs text-ink-3 mb-2">Decompiled VBA (olevba output):</p>
...
End Sub`}</pre>
          <p className="text-xs text-ink-3 mt-3">
            <code className="text-warn font-mono">AutoOpen()</code> executes automatically when the document is opened.
            The <code className="text-warn font-mono">reg add</code> command adds a value under the Run key.
          </p>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What persistence mechanism does this macro use?</p>
            <div className="flex flex-col gap-2">
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;sp00f3d_3m41l_h34d3rs&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;d3obfusc4t3d_ph1sh1ng_url&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;m4cr0_r3g1stry_p3rs1st3nc3&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
