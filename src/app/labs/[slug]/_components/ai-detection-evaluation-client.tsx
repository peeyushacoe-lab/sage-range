  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Question the Headline Metric" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A vendor pitches their AI detection engine:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{VENDOR_CLAIM}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Why is "98% accuracy" meaningless on its own here?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — on this imbalanced dataset, even a trivial always-benign model beats 98% accuracy. Flag: SAGE&#123;4ccur4cy_m1sl34d1ng_1mb4l4nc3d_d4t4&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Pick the Right Metrics" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">You need metrics that actually reflect performance on rare attack events.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Given the class imbalance, which two metrics actually matter for evaluating this detector?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — precision and recall surface what accuracy hides on imbalanced data. Flag: SAGE&#123;prec1s10n_4nd_rec4ll_m4tt3r&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Weigh the Real-World Cost" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">Testing the vendor's engine against your labeled dataset:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{RESULTS}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">The detector has high recall but a very high false-positive rate. What's the real-world cost of that?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;4ccur4cy_m1sl34d1ng_1mb4l4nc3d_d4t4&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;prec1s10n_4nd_rec4ll_m4tt3r&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4l3rt_f4t1gu3_fr0m_f4ls3_p0s1t1v3s&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
