  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Understand the Vector" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — the malicious code arrived through a legitimately signed, trusted update, making it exceptionally hard to catch. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Reassess Code Signing" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — signing proves the vendor's origin, not that their build pipeline itself is uncompromised. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
