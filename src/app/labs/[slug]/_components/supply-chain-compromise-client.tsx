  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Understand the Vector" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A vendor's public breach disclosure explains what happened:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{DISCLOSURE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What made this attack unusually hard to detect at the time it happened?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — the malicious code arrived through a legitimately signed, trusted update, making it exceptionally hard to catch. Flag: SAGE&#123;tru5t3d_51gn3d_upd4t3_b4ckd00r&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Reassess Code Signing" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">A colleague says "the update was signed, so it must be safe."</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Given that the malicious code came through a trusted, signed update, what should organizations assume about detection based on code signing alone?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — signing proves the vendor's origin, not that their build pipeline itself is uncompromised. Flag: SAGE&#123;s1gn1ng_n0t_suff1c13nt_4l0n3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Scope the Blast Radius" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">The bad update version 4.7.2 was distributed to all customers between March 3 and 19.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the right way to scope the blast radius of this kind of compromise?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;tru5t3d_51gn3d_upd4t3_b4ckd00r&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;s1gn1ng_n0t_suff1c13nt_4l0n3&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4ll_1nst4lls_4ss3ss3d_4s_c0mpr0m1s3d&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
