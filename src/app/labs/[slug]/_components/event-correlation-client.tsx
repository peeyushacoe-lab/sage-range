  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Find the Common Thread" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — Logon ID 0x3E7A21 appears in all three entries, tying them to one session. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Read the Merged Story" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — one session, logon on Host A, SMB pivot to Host B, privilege escalation on Host C. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
