  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Identify the Attack" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — this is an evil-twin AP designed to trick users into connecting via a near-identical SSID. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Locate the Device" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — triangulating signal strength from multiple points narrows down where the physical device is hidden. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
