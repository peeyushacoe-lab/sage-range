  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Compare the Techniques" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — WMI rides a legitimate management channel, leaving the stealthiest footprint. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Detect PsExec" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — Event ID 7045 (service creation), often named PSEXESVC, is the giveaway. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
