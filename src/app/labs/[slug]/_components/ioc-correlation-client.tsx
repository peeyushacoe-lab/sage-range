  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Find the Common Thread" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Three internal alerts, and a threat feed match:</p>
        ...
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What does the shared C2 domain across all three alerts confirm?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — three different hosts beaconing to the same known C2 confirms one coordinated campaign. Flag: SAGE&#123;s1ngl3_c4mp41gn_sh4r3d_c2&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Correlate the Right Way" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">A junior analyst wants to treat these as three unrelated incidents since the source IPs differ.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the flaw in that reasoning, and what's the right approach?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — correlating by shared IOC, not source host, reveals the true single campaign. Flag: SAGE&#123;c0rr3l4t3_by_sh4r3d_10c_n0t_src&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Act on the Correlation" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You've confirmed the three alerts are one campaign.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Once correlated, what's the right next step?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;s1ngl3_c4mp41gn_sh4r3d_c2&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;c0rr3l4t3_by_sh4r3d_10c_n0t_src&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;p1v0t_0n_c0nf1rm3d_10cs&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
