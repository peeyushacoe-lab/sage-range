  ...
  return (
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Spot the Escalation" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">CloudTrail logs a burst of API calls from a single access key.</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{CLOUDTRAIL_EVENTS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the new username created and the policy attached to it.</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — "backup-svc2" was created and immediately granted full AdministratorAccess, then given its own access key — a textbook cloud persistence backdoor. Flag: SAGE&#123;b4ckup_svc2_4dm1n_b4ckd00r&#125;</p>
        )}
      </TaskShell>

      {/* Task 2 */}
      <TaskShell number={2} title="Confirm Compromise via Location" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">The account's normal usage pattern:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{NORMAL_LOCATION}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What confirms this key is compromised, not just being misused internally?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — 103.22.14.9 is nowhere near the 10.40.0.0/16 office VPN range this key normally uses, combined with actions it has never performed before. Strong compromise signal. Flag: SAGE&#123;s0urc3_1p_4n0m4ly&#125;</p>
        )}
      </TaskShell>

      {/* Task 3 */}
      <TaskShell number={3} title="Remediate" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You've confirmed the key is compromised and a backdoor admin account exists.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the full remediation?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;b4ckup_svc2_4dm1n_b4ckd00r&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;s0urc3_1p_4n0m4ly&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;r3v0k3_4nd_d3l3t3_b4ckd00r&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
