    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Rogue Process" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">
          Volatility&apos;s <code className="font-mono text-warn">pslist</code> plugin enumerates running processes.
        ...
        <div className="rounded-lg bg-surface-0 border border-edge overflow-hidden mb-4">
          <p className="font-mono text-xs text-ink-3 px-4 py-2 border-b border-edge-subtle">$ vol.py -f memory.dmp pslist</p>
          <table className="font-mono text-xs w-full">
            <thead>
              <tr className="text-ink-3 border-b border-edge bg-surface-1">
                ...
                <>
                  <tr
                    ...
                    className={`cursor-pointer transition-colors hover:bg-surface-2 ${
                      p.verdict === "malicious" ? "bg-danger-wash" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-2 text-right text-ink-3 text-[10px]">
                      {expandedPid === p.pid ? "▲" : "▼"}
                    </td>
                  </tr>
                  {expandedPid === p.pid && (
                    <tr key={`${p.pid}-detail`}>
                      <td colSpan={5} className="px-4 py-3 bg-surface-1 border-t border-edge-subtle">
                        <div className="flex items-start gap-3">
                          <span className={`shrink-0 rounded border px-2 py-0.5 text-[9px] font-bold uppercase tracking-widest mt-0.5 ${
                            p.verdict === "malicious" ? "border-danger-edge bg-danger-wash text-danger" :
                            p.verdict === "suspicious" ? "border-warn-edge bg-warn-wash text-warn" :
                            "border-ok-edge bg-ok-wash text-ok"
                          }`}>
                            {p.verdict === "malicious" ? "Malicious" : p.verdict === "suspicious" ? "Suspicious" : "Clean"}
                          </span>
                          <p className="text-xs text-ink-2 whitespace-pre-wrap leading-relaxed">{p.detail}</p>
                        </div>
                      </td>
                    </tr>
                  )}
        ...
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which process is suspicious and why?</p>
      ...
      <TaskShell number={2} title="Network Connections" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">
          The <code className="font-mono text-warn">netscan</code> plugin lists active network connections.
          Click the highlighted row to see full threat context.
        </p>
        <div className="rounded-lg bg-surface-0 border border-edge overflow-hidden mb-4">
          <p className="font-mono text-xs text-ink-3 px-4 py-2 border-b border-edge-subtle">$ vol.py -f memory.dmp netscan</p>
          <div className="divide-y divide-edge-subtle font-mono text-xs">
            <div className="px-4 py-2 text-ink-3 grid grid-cols-4 gap-2">
            ...
            <div
              className="px-4 py-2 grid grid-cols-4 gap-2 cursor-pointer hover:bg-danger-wash transition-colors bg-danger-wash"
              onClick={() => setExpandedNet((v) => !v)}
            >
              <span className="text-danger font-semibold">svchost32.exe</span>
              <span className="text-danger">1337</span>
              <span className="text-danger">185.220.101.47:4444</span>
              <span className="text-danger flex items-center gap-2">ESTABLISHED <span className="text-ink-3">{expandedNet ? "▲" : "▼"}</span></span>
            </div>
            {expandedNet && (
              <div className="px-4 py-3 bg-surface-1 border-t border-danger-edge">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                  <div>
                    <p className="text-ink-3 mb-2">Connection Detail</p>
                    <div className="space-y-1">
                      <p><span className="text-ink-3">Local: </span><span className="text-warn">127.0.0.1:49823</span></p>
                      <p><span className="text-ink-3">Remote: </span><span className="text-danger">185.220.101.47:4444</span></p>
                      <p><span className="text-ink-3">Process: </span><span className="text-danger">svchost32.exe (PID 1337)</span></p>
                      <p><span className="text-ink-3">Protocol: </span><span className="text-warn">TCP</span></p>
                    </div>
                  </div>
                  <div>
                    <p className="text-ink-3 mb-2">Threat Intelligence</p>
                    <div className="space-y-1">
                      <p className="text-danger">185.220.101.47 — Known Tor exit node</p>
                      <p className="text-warn">Port 4444 — Default Metasploit RAT port</p>
                      <p className="text-ink-2">This is an active C2 channel from the rogue process</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What is the C2 IP:port? (e.g. 1.2.3.4:5678)</p>
      ...
      <TaskShell number={3} title="Code Injection" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">
          The <code className="font-mono text-warn">malfind</code> plugin scans for memory regions with
        ...
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <p className="font-mono text-xs text-ink-3 mb-2">$ vol.py -f memory.dmp malfind</p>
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{MALFIND_OUTPUT}</pre>
          <div className="mt-3 pt-3 border-t border-edge-subtle grid grid-cols-2 gap-3 text-xs">
            <div>
              <p className="text-ink-3 mb-1">PAGE_EXECUTE_READWRITE</p>
              <p className="text-ink-2">Writable AND executable — no legitimate allocation needs both. Classic injection flag.</p>
            </div>
            <div>
              <p className="text-ink-3 mb-1">MZ Header (0x4D5A)</p>
              <p className="text-ink-2">A full PE image was mapped into explorer.exe&apos;s memory — the original code was replaced.</p>
            </div>
          </div>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What injection technique is indicated by the MZ header in explorer.exe&apos;s VAD?</p>
            <div className="flex flex-col gap-2">
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;r0gu3_pr0c3ss_d3t3ct3d&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;c2_c0nn3ct10n_1d3nt1f13d&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;pr0c3ss_h0ll0w1ng_d3t3ct3d&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
