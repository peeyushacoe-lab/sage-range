  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Classify the Activity" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">Observed TTPs from a recent intrusion:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{TTPS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What class of activity does this TTP combination represent — commodity crimeware or a targeted APT-style intrusion?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — tailored targeting, patient dwell time, and sector focus point to a targeted APT-style intrusion. Flag: SAGE&#123;t4rg3t3d_4pt_styl3_1ntrus10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Explain the Value of Profiling" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">You've matched these TTPs to a known reported threat actor profile, though not with certainty.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why is matching TTPs to a known actor profile useful even without 100% certainty?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — profile matching helps anticipate next moves and apply known mitigations, even under uncertainty. Flag: SAGE&#123;4nt1c1p4t3_n3xt_m0v3s_fr0m_pr0f1l3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Apply a Healthy Caution" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">A colleague wants to name a specific actor group in the final report based on the TTP match alone.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's a caution when attributing this intrusion to a specific named actor?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;t4rg3t3d_4pt_styl3_1ntrus10n&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;4nt1c1p4t3_n3xt_m0v3s_fr0m_pr0f1l3&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;ttp_0v3rl4p_n0t_pr00f&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
