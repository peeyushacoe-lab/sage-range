...
{"admin": true, "internal_note": "ssrf_confirmed_admin_access"}`}</pre>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">
            Correct — SSRF enables access to internal services. Flag: {revealed.task_1 ?? "SAGE{…}"}
          </p>
        )}
      </TaskShell>
...
HTTP/1.1 200 OK
{
  ...
  "Token": "AQoDYXdzEJ..."
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">
            Correct — 169.254.169.254 is the link-local metadata endpoint. Flag: {revealed.task_2 ?? "SAGE{…}"}
          </p>
        )}
      </TaskShell>
...
# But 0x7f000001 is NOT in the denylist...`}</pre>
        ...
        {done("task_3") && (
          <p className="text-sm font-mono text-sage-400">
            Correct — 0x7f000001 (or decimal 2130706433) bypasses naive string filters. Flag: {revealed.task_3 ?? "SAGE{…}"}
          </p>
        )}
      </TaskShell>
