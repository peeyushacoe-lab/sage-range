  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Spot the Hunting Signal" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — one account authenticating to five different hosts inside two minutes is a strong lateral movement signal. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Confirm the Finding" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — baseline comparison plus corroborating process activity confirms this isn't normal automation. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
