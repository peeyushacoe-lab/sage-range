  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Spot the Hunting Signal" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">An authentication log pulled during a proactive hunt:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{AUTH_LOG}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What's the strongest hunting signal here?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — one account authenticating to five different hosts inside two minutes is a strong lateral movement signal. Flag: SAGE&#123;1_4cc0unt_5_h0sts_2_m1n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Confirm the Finding" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">A skeptical teammate suggests this could just be a scheduled automation job.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What additional evidence would confirm this is lateral movement and not normal automation?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — baseline comparison plus corroborating process activity confirms this isn't normal automation. Flag: SAGE&#123;b4s3l1n3_4nd_pr0c3ss_c0nf1rm&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Contain the Spread" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">Lateral movement is confirmed across five hosts, including the domain controller.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the priority containment action once lateral movement is confirmed?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;1_4cc0unt_5_h0sts_2_m1n&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;b4s3l1n3_4nd_pr0c3ss_c0nf1rm&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;d1s4bl3_4cc0unt_1s0l4t3_h0sts&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
