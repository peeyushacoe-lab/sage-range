  ...
  return (
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Read the Report" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A suspicious attachment's hash comes back with this VirusTotal report:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{VT_REPORT}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the malware family and its detection ratio.</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — 52 of 71 vendors identified this as Emotet, a well-known banking trojan/downloader. Flag: SAGE&#123;3m0t3t_52_0f_71_v3nd0rs&#125;</p>
        )}
      </TaskShell>

      {/* Task 2 */}
      <TaskShell number={2} title="Pivot on Infrastructure" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">You pivot to the Relations tab to check the C2 domains this sample contacts.</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{RELATION_TAB}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What makes cdn-update-service.net especially suspicious, beyond the vendor flags?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — legitimate infrastructure is rarely registered days before malware starts using it; freshly-registered domains tightly clustered with malware activity are a strong indicator of purpose-built attacker infrastructure. Flag: SAGE&#123;fr3shly_r3g1st3r3d_c2&#125;</p>
        )}
      </TaskShell>

      {/* Task 3 */}
      <TaskShell number={3} title="Turn Intel into Action" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You now have two confirmed-malicious C2 domains from this investigation.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What should you do with this intelligence?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;3m0t3t_52_0f_71_v3nd0rs&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;fr3shly_r3g1st3r3d_c2&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;bl0ck_4nd_r3tr0_hunt&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
