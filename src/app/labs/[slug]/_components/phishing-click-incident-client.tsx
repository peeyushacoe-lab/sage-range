    ...
    <div className="space-y-6">
      ...
      <TaskShell number={1} title="Analyze the Phishing Email" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">
          <code className="text-warn">k.reyes@acmecorp.example</code> reports clicking a link in this email
          before realizing something was wrong.
        </p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{EMAIL_HEADERS}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the domain impersonating the company's real IT support.</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — corp-it-support.info impersonates internal IT across the From, Reply-To, and Received headers, but is a completely unrelated external domain from the real acmecorp.example. Flag: SAGE&#123;c0rp_1t_supp0rt_1nf0_l00k4l1k3&#125;</p>
        )}
      </TaskShell>

      {/* Task 2 */}
      <TaskShell number={2} title="Trace the Execution Chain" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">Browser and endpoint telemetry from the moment k.reyes clicked the link:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{BROWSER_AND_PROCESS}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Summarize what happened, in order.</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — the link served a double-extension executable (.html.exe), which on execution launched an encoded PowerShell one-liner that beaconed out to a C2 server. Flag: SAGE&#123;3x3c_ch41n_t0_c2&#125;</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={3} title="File the IOC Report" unlocked={done("task_2")} completed={done("task_3")}>
        ...
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Flag the final C2 destination (IP:port) to add to the block list.</p>
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          <h3 className="font-bold text-ok text-base">Room Complete — Full Incident Closed</h3>
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Email —</span> <span className="text-ok">SAGE&#123;c0rp_1t_supp0rt_1nf0_l00k4l1k3&#125;</span></li>
            <li><span className="text-ink-3">Execution —</span> <span className="text-ok">SAGE&#123;3x3c_ch41n_t0_c2&#125;</span></li>
            <li><span className="text-ink-3">IOC Report —</span> <span className="text-ok">SAGE&#123;198_51_100_77_c2_b34c0n&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
