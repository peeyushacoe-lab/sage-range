  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Find the Malicious Mailbox Rule" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — an auto-forward rule targeting invoice emails, hiding the originals, is the malicious feature. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Name the Technique" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — this is invoice/payment redirection fraud, using the compromised account's trust to change payment details. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
