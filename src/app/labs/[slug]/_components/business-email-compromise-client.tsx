  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Find the Malicious Mailbox Rule" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A hidden rule discovered in the compromised mailbox:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{MAIL_RULE}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What malicious mailbox feature did the attacker create to intercept invoice communications?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — an auto-forward rule targeting invoice emails, hiding the originals, is the malicious feature. Flag: SAGE&#123;4ut0f0rw4rd_1nv01c3_rul3&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Name the Technique" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">The attacker then sent this from the compromised account:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{FOLLOWUP_EMAIL}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's this specific BEC technique called?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — this is invoice/payment redirection fraud, using the compromised account's trust to change payment details. Flag: SAGE&#123;p4ym3nt_r3d1r3ct10n_fr4ud&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Race Against the Wire" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You've just discovered this and the $58,000 payment may not have cleared yet.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Besides resetting the compromised account, what should be done immediately given money may already be in transit?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;4ut0f0rw4rd_1nv01c3_rul3&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;p4ym3nt_r3d1r3ct10n_fr4ud&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;4tt3mpt_w1r3_r3c4ll_1mm3d14t3ly&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
