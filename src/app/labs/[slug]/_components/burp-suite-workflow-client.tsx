  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Intercept and Choose Your Tool" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">An intercepted request in Burp's Proxy tab:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{INTERCEPT}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which Burp tool would you use to manually resend this request with role changed to admin and observe the response?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — Repeater is Burp's tool for manual, one-off request tampering. Flag: SAGE&#123;r3p34t3r_f0r_m4nu4l_t4mp3r1ng&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Confirm the Impact" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">Result after resending with role changed in Repeater:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-danger whitespace-pre-wrap overflow-x-auto">{REPEATER_RESULT}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What vulnerability does this confirm?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — changing the role parameter grants full admin access: vertical privilege escalation. Flag: SAGE&#123;r0l3_p4r4m_pr1v_3sc4l4t10n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Automate the Attack" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">There's also a 4-digit PIN field guarding a sensitive action, with no rate limiting.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Which Burp tool would you use to automatically brute-force this PIN across all 10,000 combinations?</p>
            <div className="flex flex-col gap-2">
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;r3p34t3r_f0r_m4nu4l_t4mp3r1ng&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;r0l3_p4r4m_pr1v_3sc4l4t10n&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;1ntrud3r_f0r_p1n_bruteforce&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
