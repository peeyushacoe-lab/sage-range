  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Trace the Root Cause" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">IR timeline for a persistent, unexplained domain compromise:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{SCENARIO}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">Which single account's hash, once stolen, enables forging tickets for anyone in the domain — even deleted users?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — the KRBTGT account's hash was stolen, letting the attacker forge tickets for any identity. Flag: SAGE&#123;krbtgt_4cc0unt_h4sh_st0l3n&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Explain the Persistence" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">Individual user password resets did nothing to stop the attacker's access.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why can a Golden Ticket keep working even after the compromised user's password is reset?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — the ticket is signed by KRBTGT's hash, completely independent of the impersonated user's own password. Flag: SAGE&#123;s1gn3d_by_krbtgt_n0t_us3r_pw&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Write the Remediation" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">The organization needs a remediation that actually stops the bleeding.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the only remediation that actually invalidates existing Golden Tickets?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;krbtgt_4cc0unt_h4sh_st0l3n&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;s1gn3d_by_krbtgt_n0t_us3r_pw&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;r3s3t_krbtgt_tw1c3&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
