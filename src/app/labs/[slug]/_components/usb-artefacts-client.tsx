  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Identify the Device" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">The USBSTOR registry key for the workstation in question:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{USBSTOR_ENTRY}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What is the serial number of this device and when was it last connected?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — serial 4C530001A1B2C3D4, last connected April 9. Flag: SAGE&#123;4c530001a1b2c3d4_l4st_c0nn_apr9&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Confirm Actual Access" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-3">A shellbag entry on the same workstation references the same volume serial:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{SHELLBAG_ENTRY}</pre>
        </div>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What does this matching shellbag entry confirm?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — shellbags prove the folder was actively browsed on that specific device, going beyond a mere connection record. Flag: SAGE&#123;sh3llb4gs_c0nf1rm_4cc3ss&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Confirm Exfiltration" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-3">A DLP alert from the same window of time:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap">{DLP_LOG}</pre>
        </div>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What's the conclusion, and the correct next step?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;4c530001a1b2c3d4_l4st_c0nn_apr9&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;sh3llb4gs_c0nf1rm_4cc3ss&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;1m4g3_pr3s3rv3_3v1d3nc3&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
