  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Recover a Deleted File" unlocked completed={done("task_1")}>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-sage-400">Correct — cleanup.bat is small enough to be stored resident, so its content survives deletion right inside the MFT record. Flag: {revealed.task_1 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
      ...
      <TaskShell number={2} title="Read the Recovered Script" unlocked={done("task_1")} completed={done("task_2")}>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-sage-400">Correct — it deletes the attacker's dropped tools and clears the Security log, textbook anti-forensic cleanup. Flag: {revealed.task_2 ?? "SAGE{…}"}</p>
        )}
      </TaskShell>
