  ...
  return (
    <div className="space-y-6">
      <TaskShell number={1} title="Name the Attack Type" unlocked completed={done("task_1")}>
        <p className="text-ink-2 text-sm mb-3">A diff of the checkout page's source reveals an injected script:</p>
        <div className="rounded-lg bg-surface-0 border border-edge p-4 mb-4">
          <pre className="font-mono text-xs text-warn whitespace-pre-wrap overflow-x-auto">{INJECTED_SCRIPT}</pre>
        </div>
        {!done("task_1") && (
          <form onSubmit={submitT1} className="space-y-2">
            <p className="text-sm text-ink-2 font-medium">What is this type of web-based card-stealing attack commonly called?</p>
        ...
        {done("task_1") && (
          <p className="text-sm font-mono text-ok">Correct — this is a Magecart-style web skimmer injected directly into the checkout page. Flag: SAGE&#123;m4g3c4rt_style_sk1mm3r&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={2} title="Understand the Targeting" unlocked={done("task_1")} completed={done("task_2")}>
        <p className="text-ink-2 text-sm mb-4">The malicious script only exists on the payment page, nowhere else on the site.</p>
        {!done("task_2") && (
          <form onSubmit={submitT2} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">Why does the malicious script specifically target only the payment page rather than the whole site?</p>
        ...
        {done("task_2") && (
          <p className="text-sm font-mono text-ok">Correct — it targets exactly where card data is entered, ignoring pages with nothing valuable to steal. Flag: SAGE&#123;t4rg3ts_p4ym3nt_d4t4_p01nt&#125;</p>
        )}
      </TaskShell>

      <TaskShell number={3} title="Choose the Preventive Control" unlocked={done("task_2")} completed={done("task_3")}>
        <p className="text-ink-2 text-sm mb-4">You need a control that would have stopped this before it ever ran in a customer's browser.</p>
        {!done("task_3") && (
          <form onSubmit={submitT3} className="space-y-3">
            <p className="text-sm text-ink-2 font-medium">What control would have prevented this unauthorized script from ever loading in customers' browsers?</p>
            <div className="flex flex-col gap-2">
              {[
      ...
      {allDone && (
        <div className="rounded-lg border border-ok-edge bg-ok-wash p-5 space-y-3">
          ...
          <ul className="space-y-1 font-mono text-sm">
            <li><span className="text-ink-3">Task 1 —</span> <span className="text-ok">SAGE&#123;m4g3c4rt_style_sk1mm3r&#125;</span></li>
            <li><span className="text-ink-3">Task 2 —</span> <span className="text-ok">SAGE&#123;t4rg3ts_p4ym3nt_d4t4_p01nt&#125;</span></li>
            <li><span className="text-ink-3">Task 3 —</span> <span className="text-ok">SAGE&#123;csp_bl0cks_unauth0r1z3d_scr1pts&#125;</span></li>
          </ul>
        </div>
      )}
    </div>
