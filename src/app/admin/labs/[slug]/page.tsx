    ...
    <div className="p-8 max-w-3xl">
      ...
      <div className="flex items-start justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-ink-3 text-sm mt-1 font-mono">{lab.slug}</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-white">v{lab.version}</p>
          <p className="text-xs text-ink-3">current version</p>
        </div>
      </div>

      {staleCount > 0 && (
        <div className="rounded-xl border border-warn-edge bg-warn-wash px-4 py-3 mb-6 flex items-center gap-3">
          <span className="text-warn"><Icon name="warning" size={14} className="inline-block shrink-0" /></span>
      ...
      <div className="rounded-xl border border-edge bg-surface-1 p-5 mb-8">
        <h2 className="text-sm font-semibold text-ink-2 mb-1">Publish a New Version</h2>
      ...
      {/* Version history */}
      <div>
        <h2 className="text-xs uppercase tracking-widest text-ink-3 mb-4">Version History</h2>
        {lab.changelog.length === 0 ? (
          <p className="text-ink-3 text-sm">No version changes recorded yet.</p>
        ) : (
          <div className="space-y-2">
            {lab.changelog.map((entry) => (
              <div key={entry.id} className="rounded-lg border border-edge-subtle bg-surface-1 px-4 py-3 flex items-start gap-4">
                <div className="shrink-0 w-12 text-center">
                  <p className="text-sm font-bold text-ink-2">v{entry.version}</p>
                </div>
            ...
            <div className="rounded-lg border border-dashed border-edge-subtle px-4 py-3 flex items-start gap-4 opacity-40">
              <div className="shrink-0 w-12 text-center">
                <p className="text-sm font-bold text-ink-3">v1</p>
              </div>
              <p className="text-sm text-ink-3">Initial release</p>
            </div>
          </div>
        )}
      </div>
