  ...
  return (
    <div className="p-8 space-y-8 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <Link href="/admin/academy" className="text-xs text-ink-3 hover:text-ink-2 transition mb-1 block">← Academy</Link>
      ...
      <div className="space-y-4">
        {sheets.map(sheet => (
          <div key={sheet.id} className="rounded-xl border border-edge bg-surface-1 p-5">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-semibold text-ink">{sheet.title}</h3>
              <div className="flex gap-3">
                <button onClick={() => { setEditId(sheet.id); setEditContent(sheet.content); }} className="text-xs text-ink-3 hover:text-ink-2 transition">Edit</button>
                <button onClick={() => void remove(sheet.id)} className="text-xs text-danger/60 hover:text-danger transition">Delete</button>
              </div>
            </div>
            {sheet.description && <p className="text-xs text-ink-3 mb-2">{sheet.description}</p>}
            {editId === sheet.id ? (
              <div className="space-y-2 mt-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => void save(sheet.id)} className="bg-ok hover:bg-ok-wash text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition">Save</button>
                  <button onClick={() => setEditId(null)} className="text-xs text-ink-3 hover:text-ink-2 transition">Cancel</button>
                </div>
              </div>
            ) : (
              <pre className="text-xs text-ink-2 font-mono whitespace-pre-wrap line-clamp-4 mt-2">{sheet.content}</pre>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-edge bg-surface-1 p-5 space-y-3">
        <h2 className="text-xs text-ink-3 font-semibold uppercase tracking-wider">Add Cheat Sheet</h2>
        ...
        <button onClick={() => void add()} disabled={adding || !form.title || !form.content} className="bg-ok hover:bg-ok-wash disabled:opacity-40 text-white text-sm font-semibold px-5 py-2 rounded-lg transition">
