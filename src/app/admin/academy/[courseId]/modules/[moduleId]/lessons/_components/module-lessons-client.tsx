...
export function ModuleLessonsClient({ courseId, courseTitle, module: initMod, lessons: initLessons, quiz: initQuiz }: {
  ...
  return (
    <div className="p-8 space-y-10 max-w-3xl">
      <div>
        <Link href={`/admin/academy/${courseId}/edit`} className="text-xs text-ink-3 hover:text-ink-2 transition mb-1 block">← {courseTitle}</Link>
      ...
      <Section title="Module Details">
        <div className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-xs text-ink-2 cursor-pointer select-none">
            <input type="checkbox" checked={mod.published} onChange={e => setMod(m => ({ ...m, published: e.target.checked }))} className="rounded border-edge-strong" />
            Published
          </label>
          <div className="flex items-center gap-4">
            <button onClick={() => void saveMod()} disabled={modStatus === "saving"} className="bg-ok hover:bg-ok-wash disabled:opacity-50 text-white text-sm font-semibold px-5 py-2 rounded-lg transition">
              {modStatus === "saving" ? "Saving…" : "Save Module"}
            </button>
            {modStatus === "saved" && <span className="text-xs text-ok">Saved <Icon name="check" size={14} className="inline-block shrink-0" /></span>}
          </div>
        </div>
      </Section>
      ...
      <Section title="Lessons">
        <div className="space-y-2 mb-4">
          {lessons.length === 0 && <p className="text-sm text-ink-3 italic">No lessons yet.</p>}
          {lessons.map((l, i) => (
            <div key={l.id} className="flex items-center gap-3 bg-surface-1 rounded-lg px-4 py-3 border border-edge-subtle">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink-3">{l.durationMin} min · {l.blockCount} block{l.blockCount !== 1 ? "s" : ""} · {l.cardCount} flashcard{l.cardCount !== 1 ? "s" : ""}</p>
              </div>
              <Link href={`/admin/academy/${courseId}/modules/${mod.id}/lessons/${l.id}/edit`} className="text-xs text-ink-3 hover:text-ok transition">Edit</Link>
              <button onClick={() => void deleteLesson(l.id)} className="text-[10px] text-danger/60 hover:text-danger transition">Remove</button>
            </div>
          ))}
        </div>
        <div className="rounded-xl border border-edge bg-surface-1 p-4 space-y-3">
          <p className="text-xs text-ink-3 font-semibold uppercase tracking-wider">Add Lesson</p>
          ...
          <div className="flex gap-3 items-center">
            <input type="number" min={1} value={newLesson.durationMin} onChange={e => setNewLesson(p => ({ ...p, durationMin: Number(e.target.value) }))} className="w-20 bg-surface-1 border border-edge rounded-lg px-3 py-2 text-sm text-ink focus:outline-none" />
            <span className="text-xs text-ink-3">minutes</span>
            <button onClick={() => void addLesson()} disabled={addingLesson || !newLesson.title.trim()} className="bg-surface-3 hover:bg-surface-3 disabled:opacity-40 text-white text-xs font-semibold px-4 py-1.5 rounded-lg transition ml-auto">
      ...
      <Section title="Module Quiz">
        {!quiz ? (
          <div className="text-center py-8 rounded-xl border border-edge bg-surface-1">
            <p className="text-sm text-ink-3 mb-4">No quiz for this module yet.</p>
            <button onClick={() => void createQuiz()} disabled={creatingQuiz} className="bg-surface-3 hover:bg-surface-3 disabled:opacity-40 text-white text-xs font-semibold px-4 py-2 rounded-lg transition">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              {quiz.questions.map((q, i) => (
                <div key={q.id} className="bg-surface-1 rounded-lg px-4 py-3 border border-edge-subtle">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <p className="text-xs text-ink-3 mb-1">{i + 1}. {q.type.replace("_", " ")}</p>
                      <p className="text-sm text-ink">{q.question}</p>
                    </div>
                    <button onClick={() => void deleteQuestion(q.id)} className="text-[10px] text-danger/60 hover:text-danger transition shrink-0">Remove</button>
                  </div>
                </div>
              ))}
            </div>
            <div className="rounded-xl border border-edge bg-surface-1 p-4 space-y-3">
              <p className="text-xs text-ink-3 font-semibold uppercase tracking-wider">Add Question</p>
              ...
              <button onClick={() => void addQuestion()} disabled={addingQ || !newQ.question.trim()} className="bg-surface-3 hover:bg-surface-3 disabled:opacity-40 text-white text-xs font-semibold px-4 py-1.5 rounded-lg transition">
