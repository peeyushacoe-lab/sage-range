  ...
  return (
    <div className="p-8 space-y-8">
      <div>
        ...
        <p className="text-zinc-500 text-sm mt-1">Broadcasts shown on every student&apos;s dashboard — published items appear latest-first.</p>
      </div>
      ...
      <div className="rounded-xl border border-white/8 overflow-hidden">
        {announcements.length === 0 ? (
          <p className="text-sm text-zinc-600 p-5">No announcements yet — post one above.</p>
        ) : (
            ...
            {announcements.map((a) => (
              <div key={a.id} className="flex items-start justify-between gap-4 px-5 py-4">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-zinc-400 mt-0.5">{a.body}</p>
                  <p className="text-xs text-zinc-600 mt-1.5 font-mono">
                    ...
                    {a.href && <> · <span className="text-zinc-500">{a.href}</span></>}
                  </p>
                </div>
