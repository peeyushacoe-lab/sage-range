...
export function IncidentDeleteBtn({ slug }: { slug: string }) {
  ...
  function handleDelete() {
    startTransition(async () => {
      await fetch(`/api/admin/incidents/${slug}`, { method: "DELETE" });
      router.refresh();
    });
  }
  ...
  return (
    <span className="flex items-center gap-2">
      ...
      <button onClick={handleDelete} disabled={pending} className="text-xs text-red-400 hover:text-red-300 disabled:opacity-60">
        {pending ? "…" : "Yes"}
      </button>
