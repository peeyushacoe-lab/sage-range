      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-12">
        ...
        {isInstructor && (
          <section>
            <div className="mb-6">
              ...
              <p className="text-sm text-ink-2 mt-1">Create a class, share the join code with students, and assign labs.</p>
            </div>

            <div className="mb-6">
              <p className="text-xs text-ink-3 uppercase tracking-wider mb-2">New class</p>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {myClasses.map((c) => (
                  <Link
                    key={c.id}
                    href={`/classroom/${c.id}`}
                    className="rounded-xl border border-edge p-4 hover:border-ok-edge hover:bg-surface-2 transition flex flex-col gap-2"
                  >
                    <div className="flex items-start justify-between">
                      <h3 className="font-semibold">{c.name}</h3>
                      <span className="font-mono text-xs text-ink-3 bg-surface-1 px-2 py-0.5 rounded">{c.joinCode}</span>
                    </div>
        ...
        <section>
          <div className="mb-6">
            ...
            <p className="text-sm text-ink-2 mt-1">Enter a join code from your instructor to enroll.</p>
          </div>

          <div className="mb-6">
            <p className="text-xs text-ink-3 uppercase tracking-wider mb-2">Join a class</p>
            <JoinClassroomClient />
          </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {enrolledClasses.map(({ classroom }) => (
                ...
                >
                  ...
                  <p className="text-xs text-ink-3">{classroom._count.assignments} lab{classroom._count.assignments !== 1 ? "s" : ""} assigned</p>
                </Link>
              ))}
            </div>
          )}
