...
const GRADE_COLORS: Record<string, string> = {
  A: "text-ok border-ok-edge bg-ok-wash",
  B: "text-info border-info-edge bg-info-wash",
  C: "text-warn border-warn-edge bg-warn-wash",
  D: "text-sev-high border-sev-high-edge bg-sev-high-wash",
  F: "text-danger border-danger-edge bg-danger-wash",
};
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        {items.length === 0 ? (
          ...
          <div className="flex flex-col gap-4">
            {items.map((item) => {
              const gradeColor = item.grade ? (GRADE_COLORS[item.grade] ?? "text-ink-2 border-edge") : null;
              const path = item.submission.assessment.module.path;
              const mod = item.submission.assessment.module;

              return (
                <div key={item.id} className="rounded-xl border border-edge p-6">
                  <div className="flex items-start justify-between gap-4 mb-3">
                    <div className="min-w-0">
                      <h2 className="font-semibold text-base">{item.title}</h2>
                      <p className="text-xs text-ink-3 mt-0.5">
                        <Link href={`/paths/${path.slug}`} className="hover:text-ink-2 transition">
                          {path.title}
                        </Link>
                        {" → "}
                        {mod.title}
                  ...
                  <p className="text-sm text-ink-2 leading-relaxed mb-4">{item.description}</p>

                  {item.feedback && (
                    <div className="border-t border-edge pt-4">
                      <p className="text-xs uppercase tracking-widest text-ink-3 mb-1">Mentor Feedback</p>
                      <p className="text-sm text-ink-2 leading-relaxed">{item.feedback}</p>
                    </div>
                  )}
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    ...
                    <span className="text-xs text-ink-3 border border-edge rounded-full px-2 py-0.5">
